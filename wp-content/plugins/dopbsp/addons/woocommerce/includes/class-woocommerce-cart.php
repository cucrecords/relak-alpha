        ...
        class DOPBSPWooCommerceCart{
            ...
            function add(){
                ...
                if (!$this->validateAvailability($calendar_id, $reservation)){
                    echo 'unavailable;;;;;'.$DOPBSP->text('WOOCOMMERCE_UNAVAILABLE').' <a href="'.$woocommerce->cart->get_cart_url().'">'.$DOPBSP->text('WOOCOMMERCE_VIEW_CART').'</a>';
                    die();
                }
                ...
                foreach ($cart as $cart_item_key => $cart_item){
                    if ($cart_item['product_id'] == $product_id){
                        ...
                        if (!$this->validateOverlap($calendar_id, $product_id, $reservation_data['cart_item_key'], $reservation_data['token'], $reservation)){
                            echo 'overlap;;;;;'.$DOPBSP->text('WOOCOMMERCE_OVERLAP').' <a href="'.$woocommerce->cart->get_cart_url().'">'.$DOPBSP->text('WOOCOMMERCE_VIEW_CART').'</a>';
                            die();
                        }
                        else{
                            $wpdb->insert($DOPBSPWooCommerce->tables->woocommerce, $reservation_data);
                            echo 'success;;;;;'.$DOPBSP->text('WOOCOMMERCE_SUCCESS').' <a href="'.$woocommerce->cart->get_cart_url().'">'.$DOPBSP->text('WOOCOMMERCE_VIEW_CART').'</a>';
                            die();
                        }
                    }
                }
		...
		wp_cache_flush();
            ...
            function delete(){
                ...
                if (isset($_GET['remove_item'])){
                    if ($wpdb->delete($DOPBSPWooCommerce->tables->woocommerce, array('cart_item_key' => $_GET['remove_item']))){
            ...
            function deleteItem($cart_object){
                ...
                if (isset($_GET['dopbsp_remove_item_id'])){
                    if ($wpdb->delete($DOPBSPWooCommerce->tables->woocommerce, array('id' => $_GET['dopbsp_remove_item_id']))){
                        wc_add_notice($DOPBSP->text('WOOCOMMERCE_DELETED'), 'success');
                    }
                }
            }
            ...
            function removeQuantity($return, 
                                    $product){
                $dopbsp_woocommerce_calendar_id = get_post_meta($product->id, 
