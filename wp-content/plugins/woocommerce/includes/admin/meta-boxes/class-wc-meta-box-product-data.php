		...
		<div class="panel-wrap product_data">
			...
			<div id="general_product_data" class="panel woocommerce_options_panel"><?php
				...
				echo '<div class="options_group hide_if_grouped">';
					...
					if ( wc_product_sku_enabled() ) {
						woocommerce_wp_text_input( array( 'id' => '_sku', 'label' => '<abbr title="'. __( 'Stock Keeping Unit', 'woocommerce' ) .'">' . __( 'SKU (optional)', 'woocommerce' ) . '</abbr>', 'desc_tip' => 'true', 'description' => __( 'SKU refers to a Stock-keeping unit, a unique identifier for each distinct product and service that can be purchased.', 'woocommerce' ) ) );
					} else {
				...
				echo '<div class="options_group pricing show_if_simple show_if_external">';
					...
					// Special Price
					woocommerce_wp_text_input( array( 'id' => '_sale_price', 'data_type' => 'price', 'label' => __( 'Sale Price', 'woocommerce' ) . ' ('.get_woocommerce_currency_symbol().') (optional)', 'description' => '<a href="#" class="sale_schedule">' . __( 'Schedule', 'woocommerce' ) . '</a>') );
					...
					echo '<p class="form-field sale_price_dates_fields">
								...
								<input type="text" class="short" name="_sale_price_dates_from" id="_sale_price_dates_from" value="' . esc_attr( $sale_price_dates_from ) . '" placeholder="' . _x( 'Start:', 'placeholder', 'woocommerce' ) . ' YYYY-MM-DD or use calendar" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />
								<input type="text" class="short" name="_sale_price_dates_to" id="_sale_price_dates_to" value="' . esc_attr( $sale_price_dates_to ) . '" placeholder="' . _x( 'End', 'placeholder', 'woocommerce' ) . '  YYYY-MM-DD or use calendar" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />
								<a href="#" class="cancel_sale_schedule">'. __( 'Cancel', 'woocommerce' ) . '</a>' . wc_help_tip( __( 'The sale price will be displayed for the date range selected.', 'woocommerce' ) ) . '
							</p>';
			...
			<div id="inventory_product_data" class="panel woocommerce_options_panel">
				...
				if ( 'yes' == get_option( 'woocommerce_manage_stock' ) ) {
					...
					// manage stock
					woocommerce_wp_checkbox( array( 'id' => '_manage_stock', 'wrapper_class' => 'show_if_simple show_if_variable', 'value' => 'yes', 'label' => __( 'Manage stock', 'woocommerce' ), 'description' => __( 'Enable stock management at product level', 'woocommerce' ) ) );
			...
			<div id="advanced_product_data" class="panel woocommerce_options_panel">
				...
				<div class="options_group hide_if_external">
					<?php
						// Purchase note
						woocommerce_wp_textarea_input(  array( 'id' => '_purchase_note', 'label' => __( 'Purchase Note', 'woocommerce' ), 'desc_tip' => 'true', 'description' => __( 'Enter an optional note to add to the confirmation email for the customer.', 'woocommerce' ) ) );
					?>
				</div>
				...
				<div class="options_group">
				 ...
				 <div class="options_group reviews" style="visibility:hidden;">
					<?php
